...
let take the steps which is essential for building
...
then you can get the actual UV map
...
that give you the different unwrap options
...
yes this is not nodes
the same as previous

JR: but don't you use nodes to generate your rivets?

now if you like the model to split in certain parts
You can
use nodes for that
...
and they will appear as bumps in the texture
...
you get a little menu when unwrapping a little bar
where one can control features as distance between island
of the unwrap

JR: when i regular unwrap made a mess

but the two faces top and bottom should be independent now
the sides will be a mesh probably

JR: yes a real messy one

so add one seam line to one side of the box

JR: ah ok

then it unwrap like a line with faces on
a band
...
in what ever format you draw program use
...
In most cases . lats say one have a metal build thing
...
OK so supposed you are happy with the unwrap
...
and to the left of it a directory
...
now find the texture to use
and upload that

JR: into SL you mean?

If you have one in SL you need to download it
...
yes you doo a little (vertical) row of icons
...
under the center window there is the node window

JR: yes

and you have the principled shader set up
...
and the textures you used after
...
one needs to get it separately
...
OK the bump map should start out as a uniform gray colour 0.5, 0.5 0.5 RGB
So the process is to make a picture that can act as a bump map
we can do that in blender

JR: yes

First let is go to the texture paint mode
the panels call textures paint
the view is called texture paint

JR: ok yes

[https://gyazo.com/b51993563144c3ba47c78f823e13f6e3](https://gyazo.com/b51993563144c3ba47c78f823e13f6e3)
...
you can name it in the bar in the menu line
say bump map
"Bump Map"

OK we like some bumps
on the model
...
then we have created the basis for a bump map
...
p8
like this
you can see int he texture nose
node
there us that little icon to select textures in front of the name field
so lick that and select the texture

JR: i am behind. is the Europe thing an image texture node?

no in the node editor window
...
first i added a additional texture node
then i added a BUm node
...
but you like to make it into a bump thing

JR: so wire it to height?

so we drag the texture through a converter
...
depends on how the light falls
...
is starting out as a gray scale image
set to neutral 0.5 gray

JR: and it's kind of a double image

because you see reflections
and shadows
...
there is a huge system to set up brushes
...
i add a stand alone image texture node
...
now scroll down tot he bottom of the panel
...
you have to change the bake type
