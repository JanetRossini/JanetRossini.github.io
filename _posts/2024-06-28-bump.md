...
NS: load a  gray  map  file  and then include that



NS: IN order to use that  - the  external  option  one  need to know where to draw



JR: right

NS: and in order to get that information  the UV mapping  can be exported as a  file



NS: and one can use PS to  guide how  to draw the  bump map  So today we try  to make that
...
NS: and then  we like to have a texture  Old Brass
...
NS: yes  start with a  cube  always  good  then  find



NS: old brass texture  and add to the  cube



JR: yes

NS: the  next thing is  to  unwrap the  cube  so we have a  nice UV  system



NS: and there is  of c ouse many  ways  bu  basically one goes to the  UV editor mode



NS: "UV Editing  and then maybe apply a  few seams



NS: and  do a unwrap  or  one can  jsut  let  it  do by  it  self
...
NS: you  select the  cube  go in  edit mode



NS: select all  press U for  unwrap



NS: and you should see the  unwrapped lines on the picture frame  is all  the  cube is  selected



NS: in edit mode  all borders
...
NS: make some  wierd choices  thay  is  why  i always  operate with seams
...
NS: so  should  be  ok ?  for  now  ti is  nto a  talk about  unvrapping
...
NS: so with all the  borders of the  cube s elected  on can go  to the  image  panel
...
NS: yes  and that  picture has the  black line  of the  UV layout



NS: gray is  the objet  faces and tranparent is  no  part of the  uv  So one can open that in PS



NS: and then paint on a  new layer  the  Bump map  one like to  have
...
NS: Yes i  think  the  thing  one  do then is  to find the  right place to add thing



NS: it  can be a  big problem  like the  cube  we have



NS: what is  up and  down  and how  to  orientate  you self in the  image



NS: So often i  use  two  images  one where  i mark  elements with the  textures draw



NS: and then export  that  and then export the  UV
...
NS: Yes  Other  tome it is  obvious
...
NS: not  hard to  figure out  what is  what  If you like to make a bump map
...
NS: yes  and then  when you are happy



NS: you can insert that picture as a bump map  just before then  normal input to the  Principal shader
...
NS: uv layer on top  and some  doth



NS: shoudl be  black  really  lol  it is  holes
...
NS: yes  just the layer  else the lines  will be part of the  bumps



NS: then  when we have that file  we can import to Blender



NS: Using the  Image/Open  S  i got  somethign liek this
...
NS: in image menu  there is a  open  picture



NS: or  you can drag it  in  or  use the  texture node
...
NS: o  next is  to add the  bump  mapping  yes inthe  hamburger



NS: You  remember we  add a bump map node and an image  node  and the s etup  si like  this
...
NS: you can drag the  other nodes aside  to make  room for the   two
...
NS: there is a  field  to select the  texture



NS: open that and then you can select the  image  you  like  the little  icon
...
NS: so  if  the  image is  in  and the node is  connected
...
NS: we jhave to  controll the Bump Node  We have a several option in that



NS: to change  what we  do  The first element is the DIstance



NS: I  told you last time not to play  with  that  But was to amke  things  simpler



NS: Image you liek to make  bumps  and you have some idear of the  hight  or dept of  them



NS: then the value shoudl be set to the max value  you  like  here maybe  10 cm



NS: so the  Distance shoudl be like  0.1  and i f you notice



NS: the bump are mich  less jerky  So  one  tune  the  distance



NS: into a setting where the  bumps  hight  is  about  the  expected visually  I may  go to  0.2
...
NS: the  strength  can then  be  use t  determine  the  influence  No but  what it  does  janet
...
NS: then other thign to use is of course the  strength  so  those  values are nearly always  adjusted



NS: ontill one  is pleased wit the  result  and it  dosent affect the  other normal map
...
NS: You can alse see that the bumps  Dizzi and  i have  are a bit like  pyramids



NS: that of couse depend on the  fall of  of the  brush one  use  how the  gray is converted into hight
...
NS: If  you  fisrst can see the  effet of the bump map  on your  model  it is  super easy to make a new Normal map  Yes we can save



NS: So we  have all the  tools to add a  bumpmap  what ever in most convinient for the  situation



NS: I  used PS and Blender to do  it  and i  think i get more and more  fond of  using the  Draw on the  object directly  option for line and  doth
